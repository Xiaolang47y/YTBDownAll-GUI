...
class EnvCheckFrame(ttk.Frame):
    ...
    def run_install(self, component, distro):
        ...
        if not cmd:
            messagebox.showerror("错误", f"未找到 {component} 在 {distro} 上的安装命令")
            return
        ...
        try:
            if distro == "debian" and component == "yt-dlp":
                ...
                if result.returncode == 0:
                    messagebox.showinfo("成功", f"{component} 安装成功")
                else:
                    messagebox.showerror("失败", f"安装失败: {result.stderr}")
            else:
                # 需要终端的命令
    ...
    def browse_path(self, tool):
        path = filedialog.askopenfilename(title="选择可执行文件")
        if path:
            if tool == "yt":
...
class FirstRunWizard(tk.Toplevel):
    ...
    def __init__(self, parent, config_manager):
        ...
        self.current_step = 0
        self.steps = [self.step_welcome, self.step_env_check, self.step_cookie, self.step_finish]
    ...
    def step_env_check(self):
        frame = ttk.Frame(self, padding=20)
        frame.pack(fill=tk.BOTH, expand=True)
        
        ttk.Label(frame, text="环境检测", font=("", 14, "bold")).pack(pady=10)
        
        self.env_frame = EnvCheckFrame(frame, self.config_manager, show_buttons=True)
    ...
    def on_close(self):
        if messagebox.askyesno("确认", "确定要退出配置向导吗？"):
            self.destroy()
...
class SettingsWindow(tk.Toplevel):
    ...
    def create_widgets(self):
        ...
        self.env_check_frame = EnvCheckFrame(env_frame, self.config_manager, show_buttons=True)
        self.env_check_frame.pack(fill=tk.BOTH, expand=True)
    ...
    def browse_history_dir(self):
        path = filedialog.askdirectory(title="选择历史记录保存目录")
        if path:
    ...
    def save_settings(self):
        ...
        # 保存环境路径
        self.env_check_frame.save_paths()
...
class LinkItem(tk.Frame):
    ...
    def __init__(self, parent, url, default_checks, default_sub_langs):
        ...
        ttk.Checkbutton(self, text="视频", variable=self.video_var).pack(side=tk.LEFT, padx=5)
        ttk.Checkbutton(self, text="封面", variable=self.cover_var).pack(side=tk.LEFT, padx=5)
        ...
        ttk.Checkbutton(self, text="音频", variable=self.audio_var).pack(side=tk.LEFT, padx=5)
...
class MainApplication(tk.Tk):
    ...
    def parse_links(self):
        ...
        if not lines:
            messagebox.showwarning("警告", "没有找到有效的链接")
    ...
    def start_download(self):
        if not self.link_items:
            messagebox.showwarning("警告", "请先解析链接")
            return
        ...
        if not download_items:
            messagebox.showwarning("警告", "没有选择任何下载项")
            return
